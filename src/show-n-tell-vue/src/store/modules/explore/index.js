...
import VueCommentService from '../../../services/vue-services/comment-vue-service'
...
const vueCommentService = new VueCommentService(ServiceContainer.CommentService, ServiceContainer.AuthenticationService, router);
...
const getters = {
    ...
    currentImagePostRoute: (state, getters) => `/explore/${getters.currentImagePost.id}`
}
...
const actions = {
    ...
    async [Action.CREATE_IMAGE_POST_COMMENT]({ commit, getters }, comment) {
        if(getters.currentImagePost) {
            const createdComment = await vueCommentService.createComment(getters.currentImagePost, comment, getters.currentImagePostRoute)
            if(createdComment) {
    ...
    async [Action.UPDATE_IMAGE_POST_COMMENT]({ commit, getters }, comment) {
        if(getters.currentImagePost) {
            const updatedComment = await vueCommentService.updateComment(getters.currentImagePost, comment.id, comment.content, getters.currentImagePostRoute)
            if(updatedComment) {
    ...
    async [Action.DELETE_IMAGE_POST_COMMENT]({ commit, getters }, commentId) {
        if(getters.currentImagePost) {
            const deletedComment = await vueCommentService.deleteComment(getters.currentImagePost, commentId, getters.currentImagePostRoute)
            if(deletedComment) {
