...
function AuthEngineConstructor() {
    ...
    openidPrompt: {
      ...
      set: function (val) {
        if (typeof val !== 'string') {
          throw new Error('openidPrompt must be a string. Received ' + typeof val);
        }
        ...
        if (val) {
          ...
          values.forEach(function (v) {
            if (v == 'none' && values.length > 1) {
              throw new Error('none cannot be combined with other openidPrompt values');
            }

            if (!(v in validValues)) {
              throw new Error('invalid openidPrompt value ' + v + '. Valid values: ' + Object.keys(validValues).join(', '));
            }
          });
        }
  ...
  this.initAuth2 = function () {
    ...
    auth.then(function onFulfilled() {// Let the current user listener trigger the changes.
  ...
  this.handleUserUpdate = function (newPrimaryUser) {
    ...
    this._grantedScopeArray = this.strToScopeArray(newPrimaryUser.getGrantedScopes()); // console.log(this._grantedScopeArray);
  ...
  this.strToScopeArray = function (str) {
    ...
    for (var i = 0; i < scopes.length; i++) {
      scopes[i] = scopes[i].toLowerCase(); // Handle scopes that will be deprecated but are still returned with their old value
  ...
  this.hasGrantedScopes = function (scopeStr) {
    ...
    for (var i = 0; i < scopes.length; i++) {
      if (this._grantedScopeArray.indexOf(scopes[i]) === -1) return false;
    }
  ...
  this.updateAdditionalAuth = function () {
    ...
    if (this._needAdditionalAuth != needMoreAuth) {
      this._needAdditionalAuth = needMoreAuth; // broadcast new value
  ...
  this.attachSigninAware = function (aware) {
    if (this.signinAwares.indexOf(aware) == -1) {
      this.signinAwares.push(aware); // Initialize aware properties
  ...
  this.signIn = function () {
    ...
    promise.then(function onFulfilled(response) {
      ...
      var authResponse = newUser.getAuthResponse(); // Let the current user listener trigger the changes.
    }, function onRejected(error) {
  ...
  this.signOut = function () {
    this.assertAuthInitialized();
    gapi.auth2.getAuthInstance().signOut().then(function onFulfilled() {// Let the current user listener trigger the changes.
    }, function onRejected(error) {
      console.error(error);
    });
  };
...
Polymer({
  ...
  _updateOfflineCode: function (code) {
    if (code) {
      this.fire('google-signin-offline-success', {
        code: code
      });
    }
  }
